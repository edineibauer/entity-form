...
function checkSaveAssociacaoShowAttr() {
    if($.inArray(dicionarios[entity.name][entity.edit]['format'], ["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"]) > -1) {
...
function setFormat(val) {
    ...
    if (val === "source" || val === "sources") {
        ...
        if (["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf(val) > -1) {
            $(".relation_container").removeClass("hide");
            if(["selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf(val) === -1)
...
function getSelectInput(val) {
    ...
    else if (["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult", "publisher", "owner", "passwordRequired"].indexOf(val) > -1)
...
function checkRequiresFields() {
    var type = getType();
    return (type !== "" && $("#nome").val().length > 1 && $("#nome").val() !== "id" && (["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf(type) < 0 || $("#relation").val() !== null));
}
...
function addFilter(value) {
    ...
    $.each(dicionarios[$("#relation").val()], function (i, e) {
        ...
        if (field === e.column && ["list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult", "extend", "extend_add", "extend_mult"].indexOf(e.key) > -1)
            relation = e.relation
    });
...
function checkFilterToApply() {
    if (["list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf($("#funcaoRelation").val()) > -1) {
...
function addColumnFilter($this, entity, select) {
    ...
    $.each(dicionarios[entity], function (id, data) {
        if (["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf(data.key) < 0)
...
$(function () {
    ...
    $("#content").off("keyup change focus", "#entityName").on("keyup change focus", "#entityName", function () {
        ...
        $.each(dicionarios[entity], function (i, e) {
            if (e.column === column) {
                if (["extend", "extend_add", "extend_mult", "list", "list_mult", "selecao", "checkbox_rel", "selecao_mult", "checkbox_mult"].indexOf(e.key) > -1)
